...
class Struct7TTest : ShouldSpec({
  context("Struct7T<A, B, C, D, E, F, G>") {
    val struct1 =
      struct("TestStruct1") {
      ...
      should("decode the single field and return it in a Struct1") {
        checkAll(
          ...
          shouldNotRaise { struct1.decode(slice1) }.also { (decoded, remaining) ->
            remaining.firstIndex shouldBe slice1.firstIndex + 20U
          ...
          shouldNotRaise { struct3.decode(slice3) }.also { (decoded, remaining) ->
            remaining.firstIndex shouldBe slice3.firstIndex + 19U +
              when (s3.field7) {
                is None -> 1U
                is Some -> 5U
              }
    ...
    context(".create(field1, field2)") {
      should("return Struct2(field1, field2)") {
        checkAll(arbS1, arbS2, arbS3) { s1, s2, s3 ->
          struct1.create(s1.field1, s1.field2, s1.field3, s1.field4, s1.field5, s1.field6, s1.field7) shouldBe s1
          struct2.create(s2.field1, s2.field2, s2.field3, s2.field4, s2.field5, s2.field6, s2.field7) shouldBe s2
          struct3.create(s3.field1, s3.field2, s3.field3, s3.field4, s3.field5, s3.field6, s3.field7) shouldBe s3
        }
      }
    }
    ...
    context(".encodedLength") {
      should("should be the sum of all field lengths if all fields have a defined length") {
        struct1.encodedLength shouldBe 20U
      }
    ...
    context(".members") {
      should("should contain all fields in order") {
        struct1.members.size shouldBe 7
        ...
        struct2.members.size shouldBe 7
        ...
        struct3.members.size shouldBe 7
    ...
    context("get(fieldName)") {
      should("return the field if the name exists") {
        ...
        struct3["optionalUInt"] shouldBeSameInstanceAs struct3.members[6]
      }

      should("return null if the field does not exist") {
        checkAll(
          Arb.string(1..32, Codepoint.alphanumeric())
            .filter { it !in setOf("uint8Constant", "uint8", "uint16", "uint24", "uint32", "uint64", "version") },
        ) {
        ...
        checkAll(
          Arb.string(1..32, Codepoint.alphanumeric())
            .filter { it !in setOf("bytes", "uint8", "uint16", "uint24", "uint32", "uint64", "version") },
        ) {
        ...
        checkAll(
          Arb.string(1..32, Codepoint.alphanumeric())
            .filter { it !in setOf("uint8", "uint16", "optionalUInt", "uint24", "uint32", "uint64", "version") },
        ) {

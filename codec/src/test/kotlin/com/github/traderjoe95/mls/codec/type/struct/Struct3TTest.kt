...
class Struct3TTest : ShouldSpec({
  context("Struct3T<A, B, C>") {
    val struct1 =
      struct("TestStruct1") {
    ...
    val arbS1 = Arb.struct(Arb.constant(uint8(1U)), Arb.uShort(), Arb.uInt8())
    ...
    val arbS2 = Arb.struct(Arb.byteArray(0..1024), Arb.uInt8(), Arb.uInt16())
    ...
    val arbS3 = Arb.struct(Arb.uInt8(), Arb.uInt16(), Arb.option(Arb.uInt32()))

    context(".encode(value)") {
      should("return the concatenation of all field's encodings in order") {
        checkAll(arbS1, arbS2, arbS3) { s1, s2, s3 ->
          shouldNotRaise { struct1.encode(s1) } shouldBe s1.field1.encode() + uint16(s1.field2).encode() + s1.field3.encode()
          shouldNotRaise {
            struct2.encode(s2)
          } shouldBe
            byteArrayOf(
          ...
          shouldNotRaise { struct3.encode(s3) } shouldBe s3.field1.encode() + s3.field2.encode() +
            when (val opt = s3.field3) {
      ...
      should("decode the single field and return it in a Struct1") {
        checkAll(
          ...
          shouldNotRaise { struct1.decode(slice1) }.also { (decoded, remaining) ->
            remaining.firstIndex shouldBe slice1.firstIndex + 4U
          ...
          shouldNotRaise { struct3.decode(slice3) }.also { (decoded, remaining) ->
            remaining.firstIndex shouldBe slice3.firstIndex + 3U +
              when (s3.field3) {
                is None -> 1U
                is Some -> 5U
              }
    ...
    context(".create(field1, field2)") {
      should("return Struct2(field1, field2)") {
        checkAll(arbS1, arbS2, arbS3) { s1, s2, s3 ->
          struct1.create(s1.field1, s1.field2, s1.field3) shouldBe s1
          struct2.create(s2.field1, s2.field2, s2.field3) shouldBe s2
          struct3.create(s3.field1, s3.field2, s3.field3) shouldBe s3
        }
      }
    }
    ...
    context(".encodedLength") {
      should("should be the sum of all field lengths if all fields have a defined length") {
        struct1.encodedLength shouldBe 4U
      }
    ...
    context(".members") {
      should("should contain all fields in order") {
        struct1.members.size shouldBe 3
        ...
        struct2.members.size shouldBe 3
        ...
        struct3.members.size shouldBe 3
    ...
    context("get(fieldName)") {
      should("return the field if the name exists") {
        ...
        struct3["optionalUInt"] shouldBeSameInstanceAs struct3.members[2]
      }

      should("return null if the field does not exist") {
        checkAll(Arb.string(1..32, Codepoint.alphanumeric()).filter { it !in setOf("uint8Constant", "uint8", "uint16") }) {
          struct1[it].shouldBeNull()
        }

        checkAll(Arb.string(1..32, Codepoint.alphanumeric()).filter { it !in setOf("bytes", "uint8", "uint16") }) {
          struct2[it].shouldBeNull()
        }

        checkAll(Arb.string(1..32, Codepoint.alphanumeric()).filter { it !in setOf("uint8", "uint16", "optionalUInt") }) {

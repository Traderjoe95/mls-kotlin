...
class Struct5TTest : ShouldSpec({
  context("Struct5T<A, B, C, D, E>") {
    val struct1 =
      struct("TestStruct1") {
    ...
    val arbS1 = Arb.struct(Arb.constant(uint8(1U)), Arb.uShort(), Arb.uInt8(), Arb.uInt24(), Arb.uInt32())
    ...
    val arbS2 = Arb.struct(Arb.byteArray(0..1024), Arb.uInt8(), Arb.uInt16(), Arb.uInt24(), Arb.uInt32())
    ...
    val arbS3 = Arb.struct(Arb.uInt8(), Arb.uInt16(), Arb.uInt24(), Arb.uInt32(), Arb.option(Arb.uInt32()))
      ...
      should("decode the single field and return it in a Struct1") {
        checkAll(
          ...
          shouldNotRaise { struct1.decode(slice1) }.also { (decoded, remaining) ->
            remaining.firstIndex shouldBe slice1.firstIndex + 11U
          ...
          shouldNotRaise { struct3.decode(slice3) }.also { (decoded, remaining) ->
            remaining.firstIndex shouldBe slice3.firstIndex + 10U +
              when (s3.field5) {
                is None -> 1U
                is Some -> 5U
              }
    ...
    context(".create(field1, field2)") {
      should("return Struct2(field1, field2)") {
        checkAll(arbS1, arbS2, arbS3) { s1, s2, s3 ->
          struct1.create(s1.field1, s1.field2, s1.field3, s1.field4, s1.field5) shouldBe s1
          struct2.create(s2.field1, s2.field2, s2.field3, s2.field4, s2.field5) shouldBe s2
          struct3.create(s3.field1, s3.field2, s3.field3, s3.field4, s3.field5) shouldBe s3
        }
      }
    }
    ...
    context(".encodedLength") {
      should("should be the sum of all field lengths if all fields have a defined length") {
        struct1.encodedLength shouldBe 11U
      }
    ...
    context(".members") {
      should("should contain all fields in order") {
        struct1.members.size shouldBe 5
        ...
        struct2.members.size shouldBe 5
        ...
        struct3.members.size shouldBe 5
    ...
    context("get(fieldName)") {
      should("return the field if the name exists") {
        ...
        struct3["optionalUInt"] shouldBeSameInstanceAs struct3.members[4]
      }

      should("return null if the field does not exist") {
        checkAll(
          Arb.string(1..32, Codepoint.alphanumeric())
            .filter { it !in setOf("uint8Constant", "uint8", "uint16", "uint24", "uint32") },
        ) {
          struct1[it].shouldBeNull()
        }

        checkAll(
          Arb.string(1..32, Codepoint.alphanumeric()).filter { it !in setOf("bytes", "uint8", "uint16", "uint24", "uint32") },
        ) {
        ...
        checkAll(
          Arb.string(1..32, Codepoint.alphanumeric())
            .filter { it !in setOf("uint8", "uint16", "optionalUInt", "uint24", "uint32") },
        ) {
